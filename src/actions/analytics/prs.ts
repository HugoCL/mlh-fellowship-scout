...
function processGroupedPRs(prs: PR[]): PRAnalytics {
    ...
    const fullData: { repos: string[], prs: { date: string, [key: string]: string | number }[] } = {
...
export async function getPRAnalytics(params: { type: string, id: string, days: number }): Promise<PRAnalytics> {
